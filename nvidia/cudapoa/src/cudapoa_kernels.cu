                       ...
                       uint16_t* sorted_poa_local_edge_count_d)
{
    ...
    uint32_t input_row_idx = block_idx * max_depth_per_window;
    uint8_t* window_data = &sequences_d[input_row_idx * sequences_pitch];

    uint16_t num_sequences_in_window = num_sequences_per_window_d[block_idx];
    uint16_t* sequence_length_data = &sequence_lengths_d[block_idx * max_depth_per_window];
