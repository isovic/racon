...
class SampledAlignment {
 public:
  SampledAlignment(const SingleSequence *_seq, OldOverlapLine &_mhap) : seq(_seq), mhap(_mhap) { };
...
int ConsensusFromOverlaps(const ProgramParameters &parameters, const SequenceFile &contigs, const SequenceFile &reads, const std::map<std::string, int64_t> &qname_to_ids, const std::map<std::string, int64_t> &rname_to_ids, const std::vector<OldOverlapLine> &overlaps);
...
void PrepareAndSampleOverlaps(const SequenceFile &refs, const SequenceFile &reads,
                             const std::vector<OldOverlapLine> &sorted_overlaps, int64_t start_overlap, int64_t end_overlap, int32_t num_threads,
...
int ConsensusFromOverlapsWSampling(const ProgramParameters &parameters, const SequenceFile &contigs, const SequenceFile &reads,
                                const std::map<std::string, int64_t> &qname_to_ids, const std::vector<OldOverlapLine> &sorted_overlaps);
